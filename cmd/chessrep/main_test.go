...
func TestNewRepValidator(t *testing.T) {
	rv := NewRepValidator(chess.White, []string{"foo.pgn", "bar.pgn"})
...
func TestLoad(t *testing.T) {
	rv := NewRepValidator(chess.White, []string{"../../assets/test1.pgn"})
	err := rv.Load()
	if err != nil {
...
func TestCorruptFileLoad(t *testing.T) {
	rv := NewRepValidator(chess.Black, []string{"../../assets/test2.pgn"})
	err := rv.Load()
	if err == nil {
		t.Fatalf("rv.Load() succeeded but should have failed")
	}
